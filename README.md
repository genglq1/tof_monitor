...
│
└── data/                       # 数据目录（运行时生成）
    ├── 估值表/                   # 原始估值表文件（邮件下载解压 / 手工放入）
    ├── 估值表/文件分类/          # 分类后的文件（穿透底层表来源之一）
...
`main.py` 通过 `--mode` 选择运行模式，默认 `local`：

| 模式 | 作用 | 输入 | 输出 |
|------|------|------|------|
| `email` | 下载邮件附件 + 自动分类 | 邮件系统 | `data/_原始邮件/` + `data/邮件列表.xlsx` + `data/估值表/文件分类/` |
| `local` | 解析本地估值表，跑完整管道 | `data/估值表/` | 持仓明细 + 估值统计 + 管理人回填 + 穿透报告 |
| `classify` | 仅对已有附件重新分类 | `data/_原始邮件/` + 标准表 | `data/估值表/文件分类/` |
| `nav` | 净值分析（独立于估值管道） | `data/净值表/` | `data/output/净值分析/` |
...
data/估值表/（邮件附件解压 / 手工放入的原始估值表）
    ...
    │           ↓ 依次产出：
    │           1) 信托计划持仓_完整.xlsx + 信托计划持仓_标准.xlsx
    │           2) 管理人回填（写入完整表，依赖 MANAGER_FULLNAME.xlsx）
    │           3) 信托产品估值统计结果.xlsx
    │           4) output/穿透报告/{项目代码}_{信托名称}_{日期}_穿透报告.xlsx
    │
    ├─[可选] 仅重新分类（无需重新下载）
...
- 断点续传：下载器自动跳过已存在的文件夹。
- 下载完成后自动按「投资标的」分类到 `data/估值表/文件分类/`。
...
输入：`data/估值表/`（及配置的可选底层估值表目录 `underlying_data`）
输出（按依赖顺序，全部写入 `config.yaml` 固定路径）：

1. **持仓明细**：`data/持仓统计结果/信托计划持仓_完整.xlsx`（含管理人回填）
   + `信托计划持仓_标准.xlsx`（下游穿透 / 分类强依赖）。
2. **管理人回填**：读取完整表 → 按 `data/管理人全量池/MANAGER_FULLNAME.xlsx`
   回填「产品类型」「管理人名称」→ 写回完整表。映射文档缺失则告警跳过。
3. **估值统计**：`data/信托产品估值统计结果.xlsx`（下游穿透强依赖）。
...
已有附件、无需重新下载时，仅按「投资标的」重新分类到 `data/估值表/文件分类/`。
...
- 输入：`data/净值表/`（由 `config.yaml` 的 `nav_input` 指定，与估值表**不同**目录）。
...
```yaml
paths:
  raw_data: "data/估值表"                       # 原始估值表目录
  output: "data/output"                         # 报告输出根目录
  archive_base: "data/archive"                  # 归档根目录（可选）
  logs: "logs"
  holding_full: "data/持仓统计结果/信托计划持仓_完整.xlsx"   # 下游强依赖
  holding_std:  "data/持仓统计结果/信托计划持仓_标准.xlsx"   # 下游强依赖
  valuation_stats: "data/信托产品估值统计结果.xlsx"         # 下游强依赖
  underlying_data: "data/估值表/文件分类"        # 底层估值表目录（穿透用），未配置则不跑穿透
  manager_mapping: "data/管理人全量池/MANAGER_FULLNAME.xlsx"  # 管理人回填映射（需人工维护）
  nav_input: "data/净值表"                       # 净值表目录（nav 模式）
```
